...
export default function CostBreakdown({ calls }: { calls: (CallRecord & { campaign_id: number })[] }) {
  ...
  const billedMin = talking.reduce((s, c) => s + billedMinutes(Number(c.talk_seconds)), 0)
  const carrier = billedMin * m.carrierPerMin
  const livekit = billedMin * m.livekitPerMin
  const ai = talking.length * m.aiPerAnsweredCall
  const allIn = talking.reduce((s, c) => s + estimateCallCost(Number(c.talk_seconds)), 0)
    ...
    <Stack spacing={1.2} sx={{ fontSize: '0.8rem' }}>
      <Box sx={{ bgcolor: 'action.hover', borderRadius: 1, p: 1 }}>
        <Typography className="mono" sx={{ fontSize: '0.72rem', color: 'text.secondary', lineHeight: 1.5 }}>
          cost/call = ⌈talk ÷ inc⌉ × (carrier + LiveKit) + AI/answered
        </Typography>
        <Typography sx={{ fontSize: '0.65rem', color: 'text.disabled', mt: 0.3 }}>
          Static formula — recomputes live from call telemetry ({m.currency}, {incLabel}).
        </Typography>
      </Box>
      ...
      <Box>
        ...
        <Row label="Talking calls" value={String(talking.length)} />
        <Row label="Billed talk" value={`${billedMin.toFixed(1)} min`} />
      ...
      <Box sx={{ borderTop: '1px solid', borderColor: 'divider', pt: 0.5 }}>
        ...
        <Row label="Carrier" value={R(carrier)} sub={pctOf(carrier)} />
        <Row label="LiveKit" value={R(livekit)} sub={pctOf(livekit)} />
